...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        self.train_ds = MPIIDataset(self.options, ignore_3d=self.options.ignore_3d, is_train=True)

        self.model = HSModel(cfg, is_train = True, config.SMPL_MEAN_PARAMS).to(self.device)
        ...
        self.criterion_keypoints = JointsMSELoss(use_target_weight=cfg.LOSS.USE_TARGET_WEIGHT).to(self.device)
    ...
    def train_step(self, input_batch):
        ...
        if isinstance(outputs, list):
            loss_jointsMSE = self.criterion_keypoints(outputs[0], target, target_weight)
            for output in outputs[1:]:
                loss_jointsMSE += self.criterion_keypoints(output, target, target_weight)
        else:
            output = outputs
            loss_jointsMSE = criteself.criterion_keypointsrion(output, target, target_weight)

        # loss_jointsMSE = self.criterion_keypoints()
        ...
        loss = loss_jointsMSE + self.options.shape_loss_weight * loss_shape +\
