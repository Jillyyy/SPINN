...
from models import hmr_hr, SMPL
...
if __name__ == '__main__':
    args = parse_args()
    update_config(cfg, args) # cfg. function is done
    # TODO: merge hrnet into hmr use cfg(ymal)
    ...
    model = hmr_hr(cfg, config.SMPL_MEAN_PARAMS).to(device)
    ...
    smplify = SMPLify(step_size=1e-2, batch_size=1, num_iters=4, focal_length=constants.FOCAL_LENGTH)
    ...
    new_opt_cam_t, new_opt_joint_loss = smplify(
                                ...
                                pred_cam_t.detach(),
                                0.5 * constants.IMG_RES * torch.ones(batch_size, 2, device=device),
